...
class BiFPN(nn.Module):
    def __init__(self, num_channels, epsilon=1e-4):
        ...
        # Weight
        self.p6_w1 = nn.Parameter(nn.init.kaiming_uniform_(torch.empty(2,1), mode='fan_in', nonlinearity='relu'))
        self.p6_w1_relu = nn.ReLU()
        self.p5_w1 = nn.Parameter(nn.init.kaiming_uniform_(torch.empty(2,1), mode='fan_in', nonlinearity='relu'))
        self.p5_w1_relu = nn.ReLU()
        self.p4_w1 = nn.Parameter(nn.init.kaiming_uniform_(torch.empty(2,1), mode='fan_in', nonlinearity='relu'))
        self.p4_w1_relu = nn.ReLU()
        self.p3_w1 = nn.Parameter(nn.init.kaiming_uniform_(torch.empty(2,1), mode='fan_in', nonlinearity='relu'))
        self.p3_w1_relu = nn.ReLU()

        self.p4_w2 = nn.Parameter(nn.init.kaiming_uniform_(torch.empty(3,1), mode='fan_in', nonlinearity='relu'))
        self.p4_w2_relu = nn.ReLU()
        self.p5_w2 = nn.Parameter(nn.init.kaiming_uniform_(torch.empty(3,1), mode='fan_in', nonlinearity='relu'))
        self.p5_w2_relu = nn.ReLU()
        self.p6_w2 = nn.Parameter(nn.init.kaiming_uniform_(torch.empty(3,1), mode='fan_in', nonlinearity='relu'))
        self.p6_w2_relu = nn.ReLU()
        self.p7_w2 = nn.Parameter(nn.init.kaiming_uniform_(torch.empty(3,1), mode='fan_in', nonlinearity='relu'))
...
class EfficientNet(nn.Module):
    def __init__(self, compound_coef ):
        ...
        if compound_coef == 7:
            compound_coef = 6 
        model = EffNet.from_pretrained('efficientnet-b'+str(compound_coef),advprop=True)
...
class EfficientDet(nn.Module):
    def __init__(self, num_anchors=9, num_classes=20, compound_coef=0):
        ...
        self.conv_channels = [[40,80,192,192],[40,80,192,192],[48,88,208,208],[48,96,232,232],
                                [56,112,272,272],[64,128,304,304],[72,144,344,344],[72,144,344,344]][self.compound_coef]
        self.conv3 = nn.Conv2d(self.conv_channels[0], self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv4 = nn.Conv2d(self.conv_channels[1], self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv5 = nn.Conv2d(self.conv_channels[2], self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv6 = nn.Conv2d(self.conv_channels[3], self.num_channels, kernel_size=3, stride=2, padding=1)
        ...
        self.backbone_net = EfficientNet(self.compound_coef)
