...
class Mapper():
    def __init__(self,true_labels,pred_labels,n_classes=5,threshold=0.5):
    ...
    def Check_Img(self,ground_truth,preds):
        ...
        for pair in ind:    # for intersection pairs
            if true_cls[pair[0]]==int(pred_cls[pair[1]]):
                self.counts[true_cls[pair[0]]].append('TP') # True positive if classes match
            elif true_cls[pair[0]]!=int(pred_cls[pair[1]]):
                self.counts[true_cls[pair[0]]].append('FN')  # False negative if classes do not match
        ...
        for box in unpr_boxes:
            self.counts[true_cls[box]].append('FP') # set false positive for missed boxes
        return 

    def Check_Dataset(self):
        for ground_truth,preds in tqdm(zip(self.true_labels,self.pred_labels)):
            self.Check_Img(ground_truth,preds) 

    def Get_Metrics(self,cls):
        TP = self.counts[cls].count('TP')
        FP = self.counts[cls].count('FP')
        FN = self.counts[cls].count('FN')
        if TP == 0:
            return (0,0,0)
        precision = TP/(TP+FP)
        recall = TP/(TP+FN)
    ...
    def AP(self,cls):
        TP_tot = self.counts[cls].count('TP')
        TP = 0
        FP = 0
        FN = 0
        precision = [1]
        recall = [0]
        for q,pred in enumerate(self.counts[cls]):
